...
export class ActivityPubCollectionElement extends ActivityPubElement {
  ...
  render() {
    ...
    `;
      ...
      return html`
        ...
        <header>
          ${this._title ? html`<h2 class="title">${this._title}</h2>` : ''}
          ${this.json?.totalItems !== undefined
            ? html`<p class="count">${this.json?.totalItems}</p>`
            : ''}
        </header>
  ...
  updated(changedProperties) {
    ...
    if (changedProperties.has('json')) {
      this._title = this.json?.name ?? this.json?.summary ?? 'Collection';
      this.fetchItems();
    }
  }
  ...
  async fetchItems() {
    ...
    if (this.json.items) {
      ...
      next = this.getId(this.json.first);
      while (next &&

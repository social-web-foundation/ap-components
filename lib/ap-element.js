...
export class ActivityPubElement extends LitElement {
  ...
  getId (ref) {
    if (!ref) {
      ...
      return (ref.length > 0) ? this.getId(ref[0]) : null
  ...
  getWebPage (ref) {
    if (!ref) {
      ...
      const webPages = ref.filter(link =>
        typeof link === 'string' || (typeof link == 'object' && (!ref.mediaType || ref.mediaType === 'text/html'))
      )
      return (webPages.length > 0) ? this.getWebPage(webPages[0]) : null
    } else if (typeof ref === 'object'
        && (!ref.mediaType || ref.mediaType === 'text/html')) {
      return ref.href
    } else {
      return null
    }
  ...
  get url() {
    return this.getWebPage(this.json?.url);
  }
